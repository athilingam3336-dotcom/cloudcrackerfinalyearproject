...
export const UserProfileScreen: React.FC<UserProfileScreenProps> = ({
  ...
  const { isDesktopWeb: isDesktop, isTabletWeb: isTablet } = useAppLayout();
    ...
    <ResponsiveContainer maxWidth={MAX_ADMIN_WIDTH}>
      <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          {/* Profile Hero Card */}
          <View style={[styles.profileHeroCard, isDesktop && styles.profileHeroCardDesktop]}>
            <View style={[styles.heroLeftSection, isDesktop && styles.heroLeftSectionDesktop]}>
              <TouchableOpacity
                ...
                {(() => {
                  ...
                  return safeAvatar ? (
                    ...
                    <View style={[styles.avatarPlaceholder, isDesktop && styles.avatarPlaceholderDesktop]}>
                      <Text style={[styles.avatarInitial, isDesktop && styles.avatarInitialDesktop]}>{userInitial}</Text>
                    </View>
                  );
                })()}
              ...
              <View style={[styles.profileInfo, isDesktop && styles.profileInfoDesktop]}>
                ...
                <View style={[styles.memberBadgesRow, isDesktop && { justifyContent: 'flex-start' }]}>
            ...
            {isDesktop && (
              <View style={styles.heroRightActions}>
                ...
                >
                  ...
                  <Text style={styles.headerActionBtnText}>Edit Profile</Text>
                </TouchableOpacity>

                <TouchableOpacity
                  style={[styles.headerActionBtn, styles.headerSignoutBtn]}
                  onPress={handleLogout}
                  activeOpacity={0.8}
                >
                  <MaterialIcons name="logout" size={16} color={Colors.error} />
                  <Text style={[styles.headerActionBtnText, { color: Colors.error }]}>Sign Out</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>

          {/* Main Menu Sections: Left (App Options) vs Right (Admin Operations) */}
          <View style={[styles.mainSectionsContainer, (isDesktop && isAdmin) && styles.mainSectionsContainerDesktop]}>
            {/* Left Column Section: User / App Options */}
            <View style={[(isDesktop && isAdmin) ? styles.sectionColumnDesktop : styles.sectionColumnFull]}>
              <View style={styles.sectionTitleRow}>
                <View style={styles.sectionIconBadge}>
                  <MaterialIcons name="person-outline" size={18} color={Colors.primary} />
                </View>
                <Text style={styles.sectionTitleText}>Account & Preferences</Text>
              </View>

              <View style={[styles.menuCardsList, (isDesktop && isAdmin) && styles.menuCardsListLeftDesktop]}>
                {userMenuItems.map((item) => (
                  <TouchableOpacity
                    key={item.id}
                    style={[styles.menuCard, (isDesktop && isAdmin) && styles.menuCardLeftDesktop]}
                  ...
                  >
                    ...
                    {item.badge ? (
                      <View style={[styles.itemBadge, styles.newBadge]}>
            ...
            {/* Right Column Section: Admin Control Operations */}
            {isAdmin && (
              <View style={[(isDesktop && isAdmin) ? styles.sectionColumnDesktop : styles.sectionColumnFull, !isDesktop && { marginTop: Spacing.lg }]}>
                <View style={styles.sectionTitleRow}>
                  <View style={[styles.sectionIconBadge, { backgroundColor: Colors.secondaryContainer }]}>
                    <MaterialIcons name="admin-panel-settings" size={18} color={Colors.secondary} />
                  </View>
                  <Text style={styles.sectionTitleText}>Admin Operations</Text>
                </View>
                ...
                <View style={styles.menuCardsList}>
                  {adminMenuItems.map((item) => (
                    <TouchableOpacity
                      ...
                      activeOpacity={0.8}
                    >
                      <View style={[styles.menuIconBox, { backgroundColor: Colors.secondaryContainer }]}>
                      ...
                      <View style={[styles.itemBadge, styles.adminBadge]}>
                        <Text style={styles.itemBadgeText}>{item.badge}</Text>
                      </View>
                    </TouchableOpacity>
                  ))}
          ...
          {/* Logout Action Card (visible on mobile) */}
          {!isDesktop && (
            <TouchableOpacity style={[styles.logoutCard, { marginHorizontal: Spacing.marginMobile, marginTop: Spacing.md }]} onPress={handleLogout} activeOpacity={0.8}>
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
              ...
              {isAdmin && (
                <TouchableOpacity
                  style={[styles.modalCloseBtn, { backgroundColor: Colors.secondaryContainer, marginBottom: 8, flexDirection: 'row', justifyContent: 'center', gap: 6 }]}
                  onPress={() => {
                    setIsAboutModalVisible(false);
                    navigation.navigate('AboutManagement');
                  }}
                  activeOpacity={0.8}
                >
                  <MaterialIcons name="edit" size={16} color={Colors.onSecondaryContainer} />
                  <Text style={[styles.modalCloseBtnText, { color: Colors.onSecondaryContainer }]}>
                    Edit About Content (Admin)
                  </Text>
                </TouchableOpacity>
              )}
...
const styles = StyleSheet.create({
  ...
  mainSectionsContainerDesktop: {
    flexDirection: 'row',
    alignItems: 'stretch',
    gap: Spacing.lg,
  },
  sectionColumnDesktop: {
    flex: 1,
  },
  sectionColumnFull: {
    width: '100%',
