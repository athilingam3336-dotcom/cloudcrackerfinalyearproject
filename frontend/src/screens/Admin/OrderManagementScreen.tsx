...
export const OrderManagementScreen: React.FC<OrderManagementScreenProps> = ({
  ...
  const [allOrdersOverview, setAllOrdersOverview] = useState<AdminOrderItem[]>([]);
  const [allOrdersTotalCount, setAllOrdersTotalCount] = useState<number>(0);

  // Fetch overall orders for analytics chart overview (unfiltered)
  const fetchOrdersOverview = useCallback(async () => {
    try {
      const res = await adminService.getAdminOrders(1, 100, '', 'All', 'All');
      if (res && res.orders) {
        setAllOrdersOverview(res.orders);
        setAllOrdersTotalCount(res.total);
      }
    } catch (err) {
      console.error('Failed to fetch orders overview for chart:', err);
    }
  }, []);
          ...
          <View style={styles.titleHeaderRow}>
            ...
            <View style={styles.titleTextContainer}>
              ...
              <Text style={styles.subtitle}>
                Track, filter, and manage customer pyrotechnic orders ({allOrdersTotalCount || totalOrders} total).
              </Text>
            </View>
          </View>

          {/* Centered Analytics Chart Diagram (Pie & Bar Chart) */}
          <OrderDistributionPieChart
            orders={allOrdersOverview.length > 0 ? allOrdersOverview : orders}
            totalOrdersCount={allOrdersTotalCount || totalOrders}
            selectedOrderStatus={orderStatusFilter}
            selectedPaymentStatus={paymentStatusFilter}
            onSelectOrderStatusFilter={(st) => {
              setOrderStatusFilter(st);
              setPage(1);
            }}
            onSelectPaymentStatusFilter={(pst) => {
